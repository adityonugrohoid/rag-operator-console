...
def _ingest_file(filename: str, content: bytes) -> DocumentResponse:
    """Core ingestion logic: parse, chunk, embed, and store a file."""
    doc_id = str(uuid.uuid4())
    logger.info("Starting ingestion", extra={
        "doc_id": doc_id, "document_filename": filename
    })
...
async def ingest_document(file: UploadFile = File(...)) -> DocumentResponse:
    ...
    try:
        ...
        return _ingest_file(file.filename, content)
...
async def ingest_samples():
    """Ingest all sample .txt files from the data/documents directory."""
    samples_dir = Path("/app/data/documents")
    ...
    if not txt_files:
        raise HTTPException(status_code=404, detail="No sample .txt files found")
    ...
    for txt_path in txt_files:
        ...
        try:
            ...
            result = _ingest_file(filename, content)
            results.append({
                "filename": filename,
            ...
            logger.error("Sample ingestion failed", extra={
